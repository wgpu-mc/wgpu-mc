...
impl Entity {
    ...
    pub fn new(root: EntityPart, wgpu_state: &State, texture: Arc<BindableTexture>) -> Self {
        ...
        Self {
            ...
            texture,
            parts,
            mesh: Arc::new(wgpu_state.create_buffer(&mesh[..], BufferUsages::VERTEX)),
...
impl EntityInstances {
    ...
    pub fn upload(&self, wm: &WmRenderer) {
        ...
        let instance_vbo = Arc::new(
            wm.wgpu_state
                .create_buffer(&instances[..], BufferUsages::VERTEX),
        );

        let transform_buffer = wm.wgpu_state.create_buffer(
            &matrices[..],
        ...
        let transform_bind_group = wm.wgpu_state.create_bind_group(
            pipelines.bind_group_layouts.read().get("ssbo").unwrap(),
            &[wgpu::BindGroupEntry {
                binding: 0,
