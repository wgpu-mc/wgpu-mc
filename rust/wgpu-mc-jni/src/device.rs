...
#[jni_fn("dev.birb.wgpu.rust.WgpuNative")]
pub fn createDevice(env: JNIEnv, _class: JClass, window: jlong, native_window: jlong, width: jint, height: jint) {
    ...
    let gui_textured_shader = wm.gpu.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: None,
        source: wgpu::ShaderSource::Wgsl(include_str!("../gui_textured.wgsl").into()),
    });

    let gui_textured_pipeline_layout = wm.gpu.device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: None,
        bind_group_layouts: &[
    ...
    let blit_pipeline_layout = wm.gpu.device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: None,
        bind_group_layouts: &[
            wm.bind_group_layouts.get("texture_sampler").unwrap()
        ],
    ...
    let blit_pipeline = wm.gpu.device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        label: None,
    ...
    let gui_textured_pipeline = wm.gpu.device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        label: None,
...
pub fn dropTexture(_env: JNIEnv, _class: JClass, texture: jlong) {
    unsafe { drop(Box::from_raw(texture as *mut TextureAndView)); }
}
                ...
                match command {
                    RenderPassCommand::Draw { offset, count } => {
                        ...
                        match current_pipeline {
                            1 => render_pass.set_pipeline(&BUILTIN_PIPELINES.get().unwrap().gui_textured),
                    ...
                    RenderPassCommand::DrawIndexed { offset, count, primcount, i } => {
                        if current_pipeline == 0 {
                            continue;
                        }
                        ...
                        render_pass.draw_indexed(count..(count + primcount), offset as i32, 0..i as u32);
                        
                        buffer_binds.clear();
                        texture_binds.clear();
                        current_pipeline = 0;
                    }
                    RenderPassCommand::SetIndexBuffer { index_buffer, index_type } => {
                        render_pass.set_index_buffer(index_buffer.slice(..), match index_type {
                            IndexType::I16 => wgpu::IndexFormat::Uint16,
                            IndexType::I32 => wgpu::IndexFormat::Uint32
                        });
                    }
                    RenderPassCommand::SetVertexBuffer { vertex_buffer, index } => {
                        render_pass.set_vertex_buffer(index, vertex_buffer.slice(..));
                    }
                    RenderPassCommand::SetPipeline(id) => {
                        current_pipeline = id;
                    }
                    RenderPassCommand::BindTexture { texture, name, name_len } => {
                    ...
                    RenderPassCommand::BindBuffer { buffer, name, name_len, start, end } => {
                        let name_bytes = unsafe {
                            Box::from_raw(
                                std::ptr::slice_from_raw_parts_mut(
                                    name,
