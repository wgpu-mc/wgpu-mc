...
use crate::{entity::ENTITY_ATLAS, MinecraftResourceManagerAdapter, RENDERER, RenderMessage};
...
pub fn start_rendering(mut env: JNIEnv, title: JString) -> jlong {
    let title: String = env.get_string(&title).unwrap().into();

    log::info!("Opened window");

    let mut glfw = glfw::init(|error, message| {
        panic!("{error} {message}");
    }).unwrap();

    let (pwindow, rec) = glfw.create_window(1440, 810, "hi", WindowMode::Windowed).unwrap();

    let handle = pwindow.raw_window_handle();

    let ptr = match handle {
        RawWindowHandle::Win32(ptr) => ptr.hwnd as jlong,
        _ => unimplemented!()
    };
