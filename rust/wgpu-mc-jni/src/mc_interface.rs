...
pub fn chunk_from_java_world_chunk(env: &JNIEnv, world_chunk: &JObject) {
    ...
    let sections: Vec<JObject> = (0..section_count).map(|index| {
        env.get_object_array_element(sections, index).unwrap()
