...
use wgpu_mc::mc::block::{BlockPos, BlockstateKey, ChunkBlockState};
...
use wgpu_mc::{WindowSize, WmRenderer};
...
pub extern "system" fn Java_dev_birb_wgpu_rust_WgpuNative_startRendering(
    ...
    THREAD_POOL.set(ThreadPoolBuilder::new().num_threads(4).build().unwrap());
...
pub extern "system" fn Java_dev_birb_wgpu_rust_WgpuNative_getWindowWidth(
    ...
    RENDERER
        .get()
        .map_or(1280, |wm| wm.wgpu_state.size().0 as i32)
}
...
pub extern "system" fn Java_dev_birb_wgpu_rust_WgpuNative_getWindowHeight(
    ...
    RENDERER
        .get()
        .map_or(720, |wm| wm.wgpu_state.size().1 as i32)
}
...
pub extern "system" fn Java_dev_birb_wgpu_rust_WgpuNative_debugPalette(
    ...
    let array = unsafe {
        ((packed_integer_array as usize) as *mut PackedIntegerArray)
            .as_ref()
            .unwrap()
    };
