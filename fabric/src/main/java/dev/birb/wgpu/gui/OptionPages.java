...
public class OptionPages implements Iterable<OptionPages.Page> {
    ...
    private Page createElectrum() {
        ...
        page.add(new EnumOption.Builder<Options.Backend>()
                .setName(new LiteralText("Backend"))
                .setTooltip(new LiteralText("Graphics backend used for rendering."), true)
                .setAccessors(() -> Options.BACKEND, backend -> Options.BACKEND = backend)
                .build()
        );
        page.add(new BoolOption.Builder()
                .setName(new LiteralText("HDR"))
                .setAccessors(() -> Options.HDR, aBoolean -> Options.HDR = aBoolean)
                .build()
        );
